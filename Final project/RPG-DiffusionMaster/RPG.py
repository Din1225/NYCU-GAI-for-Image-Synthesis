...
from RegionalDiffusion_xl import RegionalDiffusionXLPipeline
from diffusers.schedulers import KarrasDiffusionSchedulers,DPMSolverMultistepScheduler
from mllm import local_llm, guarded_plan
import torch
import os
...
pipe = RegionalDiffusionXLPipeline.from_pretrained("/home/a00164/oscar50513.ii13/mg/genai/cache_models/sdxl-base-1.0",
                                                   torch_dtype=torch.float16, 
                                                   use_safetensors=True, 
                                                   variant="bf16"
                                                   )

# 讀取 prompt.txt 並逐一生成圖片
prompt_file = "/home/a00164/oscar50513.ii13/mg/genai/T2I-CompBench/examples/dataset/texture.txt"  # 改成你的 prompt.txt 路徑
output_dir = "/home/a00164/oscar50513.ii13/mg/genai/RPG-DiffusionMaster/outputs/optimized_texture"  # 改成你要存的資料夾
os.makedirs(output_dir, exist_ok=True)

with open(prompt_file, "r", encoding="utf-8") as f:
    prompt_list = [line.strip() for line in f if line.strip()]

# prompt= 'The white and red clock with a yellow roof reads 11:44.'
for idx, prompt in enumerate(prompt_list):
    print(f"---------------------- prompt {idx} ---------------------------")
    para_dict = guarded_plan(prompt)  # 取代原本的 local_llm(prompt)
    split_ratio = para_dict['Final split ratio']
    regional_prompt = para_dict['Regional Prompt']
    ...
    split_ratio = para_dict['Final split ratio']
    regional_prompt = para_dict['Regional Prompt']
    save_path = os.path.join(output_dir, f"{idx:04d}.png")  # 依行號命名
    negative_prompt = ""
