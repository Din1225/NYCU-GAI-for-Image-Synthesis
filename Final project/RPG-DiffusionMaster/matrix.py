    ...
    if (KEYCOL in prompt.upper() or KEYROW in prompt.upper()):
        ...
        if (SPLROW not in split_ratio and (KEYROW in prompt.upper()) != (KEYCOL in prompt.upper())):
            ...
            (split_ratio2r, split_ratio2) = split_l2(
                split_ratio, SPLROW, SPLCOL, indsingles=True,
        ...
        # 新增的 fallback：沒有 ADDROW/ADDCOL 時當成 1x1 區域
        split_ratio2 = split_l2(split_ratio, SPLROW, SPLCOL, map_function=ffloatd(1), split_struct=lbreaks)
        split_ratio2r = [1]
        baseratio2 = split_l2(baseratio, SPLROW, SPLCOL, map_function=ffloatd(0), split_struct=lbreaks)
